...
bool dfs(int r, int c, const vector<vector<int>>& maze, vector<vector<bool>>& visited, vector<vector<int>>& parent_r, vector<vector<int>>& parent_c, int exit_r, int exit_c) {
    ...
    for (int k = 0; k < 4; ++k) {
        ...
        //Skip this neighbor if it's Out-of-bounds
        if (nr < 0 || nr >= N || nc < 0 || nc >= M) continue;

        //Skip this neighbor if it's a wall
        ...
        //Recursive DFS, returns true when an exit is found
        if (dfs(nr, nc, maze, visited, parent_r, parent_c, exit_r, exit_c)) {
            return true;
        }
    }

    //No path from current cell
    return false;
}
